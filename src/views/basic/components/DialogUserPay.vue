      ...
      <el-header style="height: auto;overflow: hidden;padding: 0;">
        ...
        <div style="padding: 5px 20px;margin:10px 0px; background-color: #F2F6FC;">
          <el-row>
            ...
            <el-col :span="15"><el-input v-model="ruleForm.payMoney" @input="payInput" clearable
                prefix-icon="el-icon-user-solid" placeholder="请输入充值金额"></el-input></el-col>
            <el-col :span="9">
              <el-button style="margin-left: 5px;" type="success" @click="sysToPay">立即充值</el-button>
              <el-button style="margin-left: 5px;" type="primary" @click="ReadICCard">读卡</el-button>
              <el-button style="margin-left: 5px;" type="warning" @click="MakeICCard">补卡</el-button>
            </el-col>
          </el-row>
          <el-row>
...
<script>
import { AnalysisCode,WriteCardOver,CreateUserCard } from '@/api/Bus'
...
import { getMakeCardPage ,postMeterMakeCard} from '@/api/makeCard'
   ...
   async ReadICCard(){
    ...
    await ICRead().then(res=>{
        ...
        AnalysisCode(res).then(res1=>{
          ...
          if(that.ICCard.ReadData._cardType != 3){
            this.$message.error('该卡不是用户卡，请使用用户卡充值');
            that.ICCard.OkWrite = false;
          }else if(that.ICCard.ReadData.data.insert1 != 1){
            that.UserList = that.AllUserList.filter(f =>f.userNo == that.ICCard.ReadData.data.userNO);
            that.userChange(that.UserList[0]?.id);
            this.$message.error('该卡还未插卡，请插卡后再充值');
            that.ICCard.OkWrite = false;
          }else{
            that.UserList = that.AllUserList.filter(f =>f.userNo == that.ICCard.ReadData.data.userNO);
            that.userChange(that.UserList[0]?.id);
    ...
    async MakeICCard(){
      ...
      if(!that.NowUser.id>0){
        this.$message.error('请先选择要补卡的用户');
        return;
      }
      await that.ReadICCard();
      if(that.ICCard.ReadData._cardType != 6){
            this.$message.error('请使用空卡进行补卡');
            return;
      }
      postMeterMakeCard({
        MeterID:that.NowUser.defaultMeter.id,
        Surplus:0,
        PayCount:that.NowUser.defaultMeter.payCount,
      }).then(res=>{

        CreateUserCard({
                  CardType:"01",
                  DevParam:that.NowUser.defaultMeter.digits,
                  LimitValue:that.NowUser.defaultMeter.setter.maxPayValue,
                  MeterNO:0,
                  Time:that.NowUser.defaultMeter.payCount,
                  UserNO:that.NowUser.userNo,
                  Value:0,
                  Insert:1
                }).then(restr=>{

                  console.log("wwwwwwwww",restr);
                    ICWrite(restr.result).then(()=>{
                      WriteCardOver({
                        meterId: that.NowUser.defaultMeter.id,
                        linkAddress: "0",
                        orderID: 0
    ...
    userChange(activeName) {
      ...
      let that = this;
      that.NowUser = Object.assign({},that.UserList.find(f => f.id == activeName));
    ...
    sysToPay() {
      ...
      that.NowUser = that.UserList.find(f => f.id == that.NowUser.id);
      this.$confirm('即将为用户 [' + that.NowUser.userName + '] 充值金额：' + that.ruleForm.payMoney + ', 是否继续?', '提示', {
        ...
        await that.ReadICCard();
        if(that.ICCard.OkWrite && that.ICCard.ReadData.data.userNO != that.NowUser.userNo){
          this.$message.error('此卡非['+that.NowUser.userName+']的用户卡，请核对卡片后再充值');
          return;
        }
          ...
          if (res.code == 200) {
            ...
            if(that.ICCard.OkWrite){
              CreateUserCard({
                ...
                LimitValue:that.NowUser.defaultMeter.setter.maxPayValue,
                MeterNO:0,
                Time:that.NowUser.defaultMeter.payCount+1,
                UserNO:that.NowUser.userNo,
                Value:res.result.Value,
                ReadData:that.ICCard.ReadString
              }).then(restr=>{

                console.log("wwwwwwwww",restr);
                  ICWrite(restr.result).then(res1=>{
                  WriteCardOver({
                    meterId: that.NowUser.defaultMeter.id,
                    linkAddress: "0",
                    orderID: res.result?res.result.id:0
                  })
                });
              })
            }
