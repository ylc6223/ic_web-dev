      ...
      <el-header style="height: auto;overflow: hidden;padding: 0;">
        ...
        <div style="padding: 5px 20px;margin:10px 0px; background-color: #F2F6FC;">
          <el-row :gutter="15">
            ...
            <el-col :span="6">
              <el-input v-model="Input.NewMeterNo"  clearable  prefix-icon="el-icon-user-solid" placeholder="请输入蓝牙表号"></el-input>
            </el-col>
            <el-col :span="4">
              
              <el-select v-model="Input.meterSetterID" placeholder="请选择用户类型">
                    <el-option v-for="item in MeterSetterList" :key="item.id" :label="item.showName" :value="item.id">
                    </el-option>
                  </el-select>
            </el-col>
            <el-col :span="4">
              <el-input v-model="Input.OldNowValue"  clearable  prefix-icon="el-icon-user-solid" placeholder="旧表读数"></el-input>
            </el-col>
            <el-col :span="4">
              <el-input v-model="Input.FillValue" clearable  prefix-icon="el-icon-user-solid" placeholder="补水量"></el-input>
            </el-col>
            <el-col :span="4">
              <el-button style="margin-left: 5px;" type="warning" @click="add">确认换表</el-button>
            </el-col>
          </el-row>
          <!-- <el-row>
...
import { getMeterReplacePage,postMeterReplace } from '@/api/meterReplace'
...
export default {
  ...
  methods: {
    ...
    add(){
      this.Input.UserID = this.NowUser.id;
      this.Input.OldMeterID = this.NowUser.defaultMeter.id;
      postMeterReplace(this.Input).then(res=>{
        if(res.result && res.result.cardCode){
        ICWrite(res.result.cardCode);
        }
      });
    },
