          ...
          <div style="height: calc(100% - 140px);overflow: auto;">
            <el-collapse accordion style="border: none;" :value="NowUser.id" @change="userChange">
              ...
              <el-collapse-item v-for="item in UserList" :name="item.id">
                <template slot="title">
                  {{ item.userNo }}.<span style="font-weight: bold;margin-right: 5px;">{{ item.userName }}</span> {{ item.phone }}
                    <el-tag v-if="item.defaultMeter.links.length > 0 && item.defaultMeter.links.find(f=>f.linkType == 0)" size="small" type="success" style="margin-left: 5px;">已制卡</el-tag>
                    <el-tag v-else size="small" type="danger" style="margin-left: 5px;">未制卡</el-tag>
                </template>
    ...
    <el-container>
      <el-header style="height: auto;overflow: hidden;padding: 0;">
        <el-card class="box-card">
          <el-form :model="ruleForm" :rules="rules" ref="ruleForm" size="small" label-position="right" label-width="80px"
            class="demo-ruleForm">
            <el-row :gutter="1">
              <el-col :span="5">
            ...
            <el-row :gutter="1">
              ...
              <el-col :span="6">
                <el-form-item label="开户费" prop="openMoney">
                  <el-input v-model="ruleForm.openMoney"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="6">
                <el-form-item label="预存金额" prop="payMoney">
                  <el-input v-model="ruleForm.payMoney" @input="payInput"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
          </el-form>
        ...
        <div style="padding: 5px 20px;margin:10px 0px; background-color: #F2F6FC;">
          <el-row>
            <el-col class="MoneyText" :span="18"><span>购水量：<b>{{ ruleForm.payM3 }}</b></span><span>开户费：<b>{{
              ruleForm.openMoney }}</b></span><span>预存金额：<b>{{
    ruleForm.payMoney }}</b></span><span>应收：<b style="color: red;">{{
    (ruleForm.payMoney * 1) + (ruleForm.openMoney * 1) }}</b></span></el-col>
            <el-col :span="6">
              <el-button style="float: right;margin-left: 5px;" type="success" @click="submitForm('ruleForm')">开
                户</el-button>
              <el-button style="float: right;margin-left: 5px;" type="primary" @click="ReadICCard">读 卡</el-button>
            </el-col>
          </el-row>
        </div>
      </el-header>
      <el-main style="padding: 0;">
        ...
        <el-tabs type="border-card" @tab-click="tabClick" v-model="tabName">
          ...
          <el-tab-pane label="充值记录" name="czjl">
            ...
            <el-pagination @current-change="queryTabList(tabName)" :page-size="UserOrders.pageSize" :current-page="UserOrders.page" style="text-align: center;" layout="prev, pager, next" :total="UserOrders.totalPages">
            </el-pagination>
          </el-tab-pane>
          ...
          <el-tab-pane label="补卡记录" name="bkjl">
            ...
            <el-pagination @current-change="queryTabList(tabName)" :page-size="UserMakeCard.pageSize" :current-page="UserMakeCard.page" style="text-align: center;" layout="prev, pager, next" :total="UserMakeCard.totalPages">
            </el-pagination>
          </el-tab-pane>
          <el-tab-pane label="换表记录" name="hbjl">
            ...
            <el-pagination @current-change="queryTabList(tabName)" :page-size="UserMeterReplace.pageSize" :current-page="UserMeterReplace.page" style="text-align: center;" layout="prev, pager, next" :total="UserMeterReplace.totalPages">
            </el-pagination>
...
<script>
import { AnalysisCode,WriteCardOver } from '@/api/Bus'
...
export default {
  ...
  async created() {
    let that = this;
    that.ICCard.ReadString = '';
    await that.ReadICCard();
    console.log("eeeeeeeeeee",that.ICCard.ReadString);
    that.queryUserAsync();
   ...
   async ReadICCard(){
    ...
    await ICRead().then(res=>{
        ...
        AnalysisCode(res).then(res1=>{
          ...
          if(that.ICCard.ReadData._cardType != 6){
            this.$message.error('该卡已被使用，请使用空卡开户');
            that.ICCard.OkWrite = false;
          }else{
            that.ICCard.OkWrite = true;
          }
        });
      });
    },
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(async (valid) => {
        if (valid) {
          await that.ReadICCard();
          that.ruleForm.ReadData = that.ICCard.ReadString;
          console.log("提交表单", this.ruleForm);

          postUser(this.ruleForm).then(async res=>  {
            that.NowUser = res.result.user;
            that.queryUserAsync();
            ...
            if(that.ICCard.OkWrite){
              ICWrite(res.result.cardCode).then(res1=>{
                console.log("wwwwwwwww",res1);
                WriteCardOver({
                  meterId: res.result.user.defaultMeter.id,
                  linkAddress: "0",
                  orderID: res.result.order?res.result.order.id:0
                })
              });
            }
